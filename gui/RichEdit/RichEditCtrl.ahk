#Include FormatRTF.ahk  ; Clipboard := Format("0x{:X}", 0x0400+234)
...
class RichEdit  ; WIN_V+
{
    ...
    ; ===================================================================================================================
    static RefCount    := 0                  ; Instance reference count.
    static Type        := "RichEdit"         ; The type of the control.
    static Class       := "RICHEDIT50W"      ; RichEdit 4.1.
    static DLL         := "Msftedit.dll"     ; RichEdit control DLL file.
    static Instance    := { }                ; DLL Handle and instances of this control.
    ...
    ; INSTANCE VARIABLES
    ; ===================================================================================================================
    Gui            := 0                          ; Gui Object thats own the control.
    ...
    __New(Gui, Options, Text := "")
    {
        if ( Type(this.Gui:=Gui) !== "Gui" )
            throw Exception("Invalid parameter #1.", -1, "RichEdit Class.")
        if ( Type(Options) !== "String" )
            throw Exception("Invalid parameter #2.", -1, "RichEdit Class.")
        if ( Type(Text) !== "String" )
            throw Exception("Invalid parameter #3.", -1, "RichEdit Class.")

        if ( RichEdit.RefCount == 0 )
        {
            if !( RichEdit.Instance["*"] := DllCall("Kernel32.dll\LoadLibrary","Str",RichEdit.DLL,"UPtr") )
                throw Exception("LoadLibrary Error " . A_LastError . ".", -1, "RichEdit Class.")
        }
        ++RichEdit.RefCount

        if ( RichEdit.SubclassCB == 0 )
        ...
        this.Ctrl := Gui.AddCustom("+" . Style . " +E" . ExStyle . A_Space . Options . " Class" . RichEdit.Class)
        this.wt := "ahk_id" . ( this.hWnd := this.Ctrl.hWnd )
        ...
        DllCall("Comctl32.dll\SetWindowSubclass", "UPtr", this.hWnd, "UPtr", RichEdit.SubclassCB, "UPtr", this.hWnd, "UPtr", 0)
    }
    ...
    __Delete()
    {
        DllCall("Comctl32.dll\RemoveWindowSubclass", "UPtr", this.hWnd, "UPtr", RichEdit.Subclass, "UPtr", 0)
        try CallbackFree( this.GetAutoCorrectProc() )
        DllCall("User32.dll\DestroyWindow", "UPtr", this.hWnd)
        RichEdit.Delete(this.hWnd)

        if ( RichEdit.RefCount && !--RichEdit.RefCount )
        {
            CallbackFree(RichEdit.SubclassCB)
            RichEdit.SubclassCB := 0
            DllCall("Kernel32.dll\FreeLibrary", "UPtr", RichEdit.Instance["*"])
        }
    }
    ...
    SubclassProc(uMsg, wParam, lParam, uIdSubclass, dwRefData)
    {
        ...
        return uMsg == 0x87 ? 4 : DllCall("Comctl32.dll\DefSubclassProc", "UPtr", this, "UInt", uMsg, "UPtr", wParam, "UPtr", lParam, "UPtr")
    ...
    {
        return SendMessage(0x478,, Options,, this.wt)
    ...
    */
    ...
    {
        return SendMessage(0x4CA, Values, Mask==""?Values:Mask,, this.wt)
    } ; https://docs.microsoft.com/en-us/windows/desktop/controls/em-settypographyoptions

    GetRect()
    {
        local RECT := "", _ := VarSetCapacity(RECT, 16)
        SendMessage(0xB2,, &RECT,, this.wt)
        return { left:NumGet(&RECT,"Int"), top:NumGet(&RECT+4,"Int"), right:NumGet(&RECT+8,"Int"), bottom:NumGet(&RECT+12,"Int") }
    ...
    SetRect(Rect, Relative := 0)
    {
        ...
        return SendMessage(0xB3, Relative, &_RECT,, this.wt)
    ...
    */
    ...
    {
        return SendMessage(0x44D, Operation, Options,, this.wt)
    ...
    */
    GetOptions()
    {
        return SendMessage(0x44E,,,, this.wt)
    ...
    */
    LimitText(Value)
    {
        SendMessage(0x435,, Value,, this.wt)
    ...
    */
    ...
    {
        local  LineIndex := SendMessage(0xBB, LineNum,,, this.wt)
    ...
    */
    GetLineCount()
    {
        return SendMessage(0xBA,,,, this.wt)
    ...
    */
    ...
    {
        local lnindex := SendMessage(0xBB, LineNum,,, this.wt)
    ...
    */
    ...
    {
        return SendMessage(0xC1, CharIndex,,, this.wt)
    ...
    */
    ...
    {
        return SendMessage(0xBB, LineNumber,,, this.wt)
    ...
    */
    ...
    {
        local obj := { first: SendMessage(0xBB,LineNumber,,,this.wt) }
        obj.length := SendMessage(0xC1, obj.first,,, this.wt)
    ...
    */
    ...
    {
        return SendMessage(0x436,, CharIndex,, this.wt)
    ...
    */
    CharFromPos(X, Y)
    {
        ...
        return SendMessage(0xD7,, &POINTL,, this.wt)
    ...
    */
    ...
    {
        local POINT := 0
        DllCall("User32.dll\GetCursorPos", "UPtrP", POINT)
        ,DllCall("User32.dll\ScreenToClient", "UPtr", this.hWnd, "UPtrP", POINT)
    ...
    */
    GetCaretLine()
    {
        return SendMessage(0x436,, SendMessage(0xBB,-1,,,this.wt),, this.wt)
    ...
    */
    SelectionType()
    {
        return SendMessage(0x442,,,, this.wt)
    ...
    */
    SetSelection(Pos1, Pos2 := "", Reverse := FALSE)
    {
        ...
        return SendMessage(0x437,, &CHARRANGE,, this.wt)
    ...
    */
    ...
    {
        local CHARRANGE := 0
        SendMessage(0x434,, &CHARRANGE,, this.wt)
    ...
    */
    GetSelText()
    {
        ...
        ,SendMessage(0x43E,, &Buffer,, this.wt)
    ...
    */
    ...
    /*
        ...
        Example to retrieve the first 3 characters of the selected text:
            GetText(3*2+2, 2)    ; Specify zero in the first parameter to retrieve the whole selected text.
        Remarks:
    ...
    */
    GetText(Length := -1, Mode := 0)
    {
        ...
        ,VarSetCapacity(Buffer, 0*SendMessage(0x45E,&GETTEXTEX,&Buffer,,this.wt)-1)
        return Buffer
    ...
    */
    ...
    {
        local TEXTRANGE := "", Buffer := "", _ := VarSetCapacity(TEXTRANGE, 8+A_PtrSize, 0*VarSetCapacity(Buffer,2*(Max-Min)+2,0))
        NumPut((Min&0xFFFFFFFF)|((Max&0xFFFFFFFF)<<32), &TEXTRANGE, "Int64"), NumPut(&Buffer, &TEXTRANGE+8, "UPtr")
        ,SendMessage(0x44B, 0, &TEXTRANGE,, this.wt), VarSetCapacity(Buffer, -1)
        return Buffer
    ...
    */
    GetTextLength(Mode := 0)
    {
        ...
        return SendMessage(0x45F, &GETTEXTLENGTHEX,,, this.wt)
    ...
    */
    SetText(ByRef Text, Mode := 0)
    {
        ...
        return SendMessage(0x461, &SETTEXTEX, pBuffer,, this.wt)
    ...
    */
    SetPlainText(ByRef Text)
    {
        ...
        return SendMessage(0xC,, type(Text)="integer"?Text:&Text,, this.wt)
    ...
    */
    ReplaceSel(ByRef Text, CanUndo := FALSE)
    {
        SendMessage(0xC2, CanUndo, &Text,, this.wt)
    ...
    */
    HideSelection(Mode)
    {
        SendMessage(0x043F, Mode,,, this.wt)
    ...
    */
    SetZoom(Ratio)
    {
        return SendMessage(0x4E1, Ratio, 100,, this.wt) ? Ratio : 0
    ...
    */
    GetZoom()
    {
        ...
        local R := SendMessage(0x4E0, &Numerator, &Denominator,, this.wt)
    ...
    */
    SetBkgndColor(Color)
    {
        Color := SendMessage(0x443, Color==-1?1:0, ((Color&0xFF0000)>>16)+(Color&0xFF00)+((Color&0xFF)<<16),, this.wt)
    ...
    */
    ...
    {
        return SendMessage(0x45B, Mode, lParam,, this.wt)
    ...
    */
    GetAutoURL()
    {
        return SendMessage(0x45C,,,, this.wt)
    ...
    */
    Undo()
    {
        return SendMessage(0xC7,,,, this.wt)
    ...
    */
    Redo()
    {
        return SendMessage(0x454,,,, this.wt)
    ...
    */
    Paste()
    {
        return SendMessage(0x302,,,, this.wt)
    ...
    */
    Cut()
    {
        return SendMessage(0x300,,,, this.wt)
    ...
    */
    Copy()
    {
        return SendMessage(0x301,,,, this.wt)
    ...
    */
    Clear()
    {
        return SendMessage(0x303,,,, this.wt)
    ...
    */
    CanRedo()
    {
        return SendMessage(0x455,,,, this.wt)
    ...
    */
    CanUndo()
    {
        return SendMessage(0xC6,,,, this.wt)
    ...
    */
    ClearUndo()
    {
        return this.SetUndoLimit(SendMessage(0x452,,,,this.wt)+this.UndoLimit)
    ...
    */
    SetUndoLimit(Value)
    {
        return this.UndoLimit := SendMessage(0x452, Value,,, this.wt)
    } ; https://docs.microsoft.com/en-us/windows/desktop/controls/em-setundolimit
    ...
    */
    GetUndoName()
    {
        return SendMessage(0x456,,,, this.wt)
    }
    ...
    */
    SetStyle(Flags, Mask)
    {
        return SendMessage(0x4CC, Flags, Mask,, this.wt)
    ...
    */
    FindText(ByRef Text, Min, Max, Mode := 0)
    {
        ...
        ,SendMessage(0*NumPut(&Text,&FINDTEXTEX+8,"UPtr")+0x47C, Mode, &FINDTEXTEX,, this.wt)
    ...
    */
    ...
    {
        return SendMessage(0x44C, Mode, CharIndex,, this.wt)
    ...
    */
    SetFont(Options := "", FontFace := "", Mode := 0)
    {
        ...
        return SendMessage(0x444, Mode, &CHARFORMAT2,, this.wt)
    ...
    */
    SetFontRange(Min := "", Max := "", Options := "", FontFace := "", Mode := 3)
    {
        ...
        if ( Mode & 1 )
            SendMessage(0*SendMessage(0x434,,&CHARRANGE1,,this.wt)+0x4DD,, &POINT,, this.wt)
        if ( Min !== "" && Max !="" ) {
            local CHARRANGE2 := (Min&0xFFFFFFFF)|((Max&0xFFFFFFFF)<<32)
            this.SetFont( Options, FontFace, 0*SendMessage(0x437,,&CHARRANGE2,,this.wt)+1 )
        } if ( Mode & 2 )
            SendMessage(0*SendMessage(0x437,,&CHARRANGE1,,this.wt)+0x4DE,, &POINT,, this.wt)
        return TRUE
    ...
    */
    SetFontSize(Value)
    {
        return SendMessage(0x4DF, Value,,, this.wt)
    ...
    */
    SetAlignment(Alignment)
    {
        ...
        return SendMessage(0x447, NumPut(Alignment,&PARAFORMAT2+24,"UShort"), &PARAFORMAT2,, this.wt)
    ...
    */
    SetLineSpacing(LineSpacing, Rule := 5)
    {
        ...
        ,NumPut(LineSpacing, &PARAFORMAT2+164, "Int"), NumPut(Rule,&PARAFORMAT2+170,"UChar")
        return SendMessage(0x447,, &PARAFORMAT2,, this.wt)
    } ; https://docs.microsoft.com/en-us/windows/desktop/controls/em-setparaformat
    ...
    */
    SetSpacing(Before := "", After := "")
    {
        local PARAFORMAT2
        NumPut(0*VarSetCapacity(PARAFORMAT2,188)+188, &PARAFORMAT2, "UInt")
        ,NumPut((Before==""?0:0x40)|(After==""?0:0x80), &PARAFORMAT2+4, "UInt")
        ,NumPut(After, NumPut(Before,&PARAFORMAT2+156,"Int"), "Int")
        return SendMessage(0x447,, &PARAFORMAT2,, this.wt)
    } ; https://docs.microsoft.com/en-us/windows/desktop/controls/em-setparaformat
    ...
    */
    GetParaFormat()
    {
        ...
        obj.mask := SendMessage(0x43D, NumPut(obj.size,&PARAFORMAT2,"UInt"), &PARAFORMAT2,, this.wt)
    ...
    */
    SetDefaultTabStops(TabStops, Count := -1)
    {
        ...
        this.DefTabStops := (arr:=SendMessage(0xCB,Count,&arr,,this.wt)) ? SubStr(_,1,-1) : this.DefTabStops
        return 0*this.SetRedraw(TRUE, TRUE, TRUE, TRUE) + arr
    ...
    */
    SetRedraw(Mode, Redraw := FALSE, Erase := FALSE, Update := FALSE)
    {
        local ret := !SendMessage(0xB, Mode, 0,, this.wt)
        if ( Redraw )
            ; https://docs.microsoft.com/en-us/windows/desktop/api/winuser/nf-winuser-invalidaterect
            ret := DllCall("User32.dll\InvalidateRect", "UPtr", this.hWnd, "Ptr", 0, "Int", Erase)
        if ( Update )
            ; https://docs.microsoft.com/en-us/windows/desktop/api/winuser/nf-winuser-updatewindow
            ret := DllCall("User32.dll\UpdateWindow", "UPtr", this.hWnd)
        return ret
    ...
    */
    SetEventMask(Mask, Mode := 0)
    {
        ...
        return SendMessage(0x445,, Mask,, this.wt)
    ...
    */ 
    GetEventMask()
    {
        local EventMask := SendMessage(0x445,,,, this.wt)
        return 0*SendMessage(0x445,,EventMask,, this.wt) + EventMask
    } ; https://docs.microsoft.com/en-us/windows/desktop/controls/em-seteventmask
    ...
    */
    StreamOut(Mode, Callback := 0, Cookie := 0, CallbackFree := FALSE, CodePage := 1200)
    {
        ...
        r := SendMessage(0x044A, CodePage==""?Mode:(CodePage<<16)|0x20|Mode, &EDITSTREAM,, this.wt), ErrorLevel := NumGet(&EDITSTREAM+A_PtrSize, "UInt")
    ...
    */
    StreamIn(Mode, Callback := 0, Cookie := 0, CallbackFree := FALSE)
    {
        ...
        NumPut(Callback?Callback:CallbackCreate("EditStreamCallback"), NumPut(Cookie,&EDITSTREAM,"UPtr")+4, "UPtr")
        r := SendMessage(0x0449, Mode, &EDITSTREAM,, this.wt)
        try CallbackFree( Callback && !CallbackFree ? 0 : NumGet(&EDITSTREAM+A_PtrSize+4,"UPtr") )
    ...
    */
    Load2(FileName, Mode := 1)
    {
        ...
        if ( f && f.Encoding == "UTF-16")  ; UTF-16LE (use Load method)
            ...
            DllCall("Kernel32.dll\LCMapStringW", "UInt", 0, "UInt", 0x800, "UPtr", &BE, "UInt", f.length-2, "UPtr", &LE, "UInt", f.length-2)
    ...
    */
    SetAutoCorrectProc(AutoCorrectProc)
    {
        ...
        return SendMessage(0x4EA, AutoCorrectProc,,, this.wt)
    ...
    */
    GetAutoCorrectProc()
    {
        return SendMessage(0x4E9,,,, this.wt)
    ...
    */
    ...
    {
        return SendMessage(0x4FF, WChar,,, this.wt)
    ...
    */
    InsertImage(pIStream, xWidth, yHeight, Ascent := 0, Type := 0, AlternateText := "")  ; WIN_8+
    {
        ...
        return SendMessage(0x53A,, &RICHEDIT_IMAGE_PARAMETERS,, this.wt)
    ...
    */
    ...
    Visible[]
    {
        get {
            ...
            return DllCall("User32.dll\IsWindowVisible", "UPtr", this.hWnd)
        }
        set {
            ; ShowWindow function
            ; https://msdn.microsoft.com/en-us/library/windows/desktop/ms633548(v=vs.85).aspx
            return DllCall("User32.dll\ShowWindow", "UPtr", this.hWnd, "Int", Value ? 8 : 0)
        }
    }
    ...
    Enabled[]
    {
        get {
            ...
            return DllCall("User32.dll\IsWindowEnabled", "UPtr", this.hWnd)
        }
        set {
            ; EnableWindow function
            ; https://msdn.microsoft.com/en-us/library/windows/desktop/ms646291(v=vs.85).aspx
            return DllCall("User32.dll\EnableWindow", "UPtr", this.hWnd, "Int", !!Value)
        }
    }
}
